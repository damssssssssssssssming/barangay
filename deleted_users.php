...
$kweri = "SELECT * FROM deleted_user RIGHT JOIN deleted_infos ON deleted_user.id=deleted_infos.user_id";
$kweried = mysqli_query($conn, $kweri);
if(isset($_GET['submit'])) {
    $user_id = $_GET['search'];
    $kweri = "SELECT * FROM deleted_user RIGHT JOIN deleted_infos ON deleted_user.id = deleted_infos.user_id WHERE deleted_user.id = $user_id";
    $kweried = mysqli_query($conn, $kweri);
}
...
<html lang="en">
  ...
  z-index: 1; "><?php echo $number1 ?></span> <!-- Overlay text -->
        ...
        <br>
        <br>
        <h1 style="position: relative; right: -20px;">Deleted User's Account</h1>
        <br>
        <center>
        ...
        <?php while ($kwerieded = $kweried->fetch_assoc()): ?>
            <div class="poool">
                ...
                <form action="deleted_more_infos.php" method="post">
                <div class="poool4">
                ...
                <div class="poool4">
                    ...
                    <input type="button" onclick="popup1(<?php echo $kwerieded['user_id'] ?>)" style="width: 100%; height: 100%; font-size: 20px; background-color: #71F236; text-align: center;" value="Restore this User's Account">
                </div>
            </div>
            <br>
            <?php endwhile; ?>
            <div class="popup" id="popup">
                    ...
                    <h2>Are you sure you want to restore this User's account?: </h2>
                        <input style="display: none" name="id" id="hello" required>
